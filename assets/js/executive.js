/* ==========================================================================
   executive.js â€” the read-only side: everything computed from school records
...
TVET.Executive = (function () {
  ...
  function renderTimeline() {
    ...
    $("elapsedText").textContent = p.elapsedMonths + " of " + p.months + " months Â· " + share + "% elapsed";
  }
  ...
  function renderKpis(t) {
    $("execKpis").innerHTML =
      ...
      U.kpi(U.fmt(t.enrolled), "Students reached",
            D.pct(t.female, t.enrolled) + "% female Â· " + U.fmt(t.inclusion) + " under inclusion criteria") +
      U.kpi(D.pct(t.tTrained, t.tTotal) + "%", "Teachers trained",
            U.fmt(t.tTrained) + " of " + U.fmt(t.tTotal) + " in post Â· " + U.fmt(t.tCert) + " certified") +
      U.kpi(D.pct(t.installed, t.delivered) + "%", "Equipment installed",
            U.fmt(t.installed) + " of " + U.fmt(t.delivered) + " computers Â· " + U.fmt(t.labs) + " labs");
  }

  function renderProgress(t) {
    $("execProgress").innerHTML = '<h3 class="card__title">Objectives</h3><div class="proglist">' +
      U.progress("1 Â· ICT infrastructure", "Computers installed against delivered",
                 D.pct(t.installed, t.delivered),
                 U.fmt(t.installed) + " of " + U.fmt(t.delivered)) +
      U.progress("2 Â· Teacher capacity", "Teachers trained in blended learning",
                 D.pct(t.tTrained, t.tTotal),
                 U.fmt(t.tTrained) + " of " + U.fmt(t.tTotal)) +
      U.progress("3 Â· Data systems", "Schools using the Quality Monitoring Dashboard",
  ...
  function renderFlags() {
    ...
    host.innerHTML =
      '<h3 class="card__title">Open flags Â· ' + all.length + ' (' + high + ' high)</h3>' +
      '<ul class="flaglist">' + shown.map(function (f) {
        return '<li class="sev-' + f.sev + '"><b>' + U.esc(f.subject) + '</b> â€” <span>' +
               U.esc(f.why) + '</span></li>';
      }).join("") + '</ul>' +
      (all.length > MAX_FLAGS
        ? '<p class="flaglist__more">' + (all.length - MAX_FLAGS) +
          ' further flags â€” see the school-by-school table below.</p>'
        : "");
  }
  ...
  function renderProvinces() {
    ...
    rows.forEach(function (r) {
      html += '<tr>' +
        ...
        '<td class="right num">' + (r.count ? U.fmt(r.enrolled) : "â€”") + '</td>' +
        '<td class="right num">' + (r.tTotal ? D.pct(r.tTrained, r.tTotal) + "%" : "â€”") + '</td>' +
        '<td class="right num">' + (r.delivered ? D.pct(r.installed, r.delivered) + "%" : "â€”") + '</td>' +
        '</tr>';
    });
  ...
  function renderSchools() {
    ...
    ranked.forEach(function (s) {
      ...
      html += '<tr>' +
        ...
        '<td class="muted nowrap">' + U.esc(s.connectivity) + ' network Â· ' + U.esc(s.power) + ' power</td>' +
